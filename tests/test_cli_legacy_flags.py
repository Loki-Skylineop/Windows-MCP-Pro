...
@pytest.mark.parametrize(
    ("args", "suggestion"),
    [
        (["--transport", "sse"], "windows-mcp-pro serve --transport sse"),
        (["--debug"], "windows-mcp-pro serve --debug"),
        (["--port", "8000"], "windows-mcp-pro serve --port 8000"),
    ],
)
def test_legacy_serve_flags_at_group_level_get_migration_hint(args, suggestion):
    ...
    assert "`windows-mcp-pro` is now a command group" in result.output
    assert suggestion in result.output
    assert "windows-mcp-pro serve --help" in result.output
...
def test_install_options_are_not_blocked_by_legacy_filter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wm, "_resolve_program", lambda: ["windows-mcp-pro"])
    ...
    assert "windows-mcp-pro serve --transport sse" in (tmp_path / "start-server.cmd").read_text(
        encoding="utf-8"
    )
